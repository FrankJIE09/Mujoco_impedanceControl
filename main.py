...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while data.time < DURATION:
        ...
        mujoco.mj_jac(model, data, jacp, jacr, data.geom_xpos[-1], 6)
        r = Rotation.from_matrix(data.geom_xmat[-1].reshape(3, 3))
        rpy = r.as_euler('xyz', degrees=True)
        error = np.append(data.geom_xpos[-1], rpy) - init_pose
        error_array = np.vstack((error_array, error))

        F = k * error_array[-1] + b * np.diff(error_array, axis=0)[-1]/dt #+m*np.diff(error_array,n=2, axis=0)[-1]/dt
        jacb = np.vstack((jacp, jacr))
        # print(data.geom_xpos[-1])
        v = -F
        # data.ctrl = 0
        qvel = np.linalg.pinv(jacb).dot(v)
        # Step the simulation.
